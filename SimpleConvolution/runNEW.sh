...
for I in {1..15} 
do
/opt/AMDAPPSDK-2.9-1/samples/opencl/cl/wattsup/wattsup.py -p /dev/ttyUSB0 -t 5 -i 1 -y > SimpleConvolution_0_${I}.csv & 
sleep 1s
./../SimpleConvolution/bin/x86_64/Release/SimpleConvolution -e -ndeg 0 -wic 128 -k 0 > out_0_128_${I}.txt
sleep 3s
killall wattsup.py
sleep 1s
sudo mv SimpleConvolution_0_${I}.csv "$RDIR"	
...
echo "test for $NUMFAULTS degraded elements ... "
	for I in {1..15} 
	do
		/opt/AMDAPPSDK-2.9-1/samples/opencl/cl/wattsup/wattsup.py -p /dev/ttyUSB0 -t 5 -i 1 -y > SimpleConvolution_${NUMFAULTS}_${I}.csv & 
		sleep 1s
		...
		killall wattsup.py
		sleep 1s
		sudo mv SimpleConvolution_${NUMFAULTS}_${I}.csv "$RDIR"	
